...
const useMember = () => {
    ...
    const handleClickPatch = async (email: string, currentState: string) => {
        const status = currentState === "ACTIVATE" ? "DEACTIVATE" : "ACTIVATE";

        try {
            const response = await babyaAxios.patch(`/member/status/${email}/${status}`);
            if (response.status === 200) {
                showToast("success", "회원 상태가 성공적으로 변경되었습니다.");

                setMemberList((prevList) =>
                    prevList.map((member) =>
                        member.email === email ? { ...member, state: status } : member
                    )
                );
            } else {
                showToast("error", "회원 상태 변경에 실패했습니다.");
            }
        } catch (error) {
            console.error(error);
            if (axios.isAxiosError(error) && error.response) {
                if (error.response.status === 404) {
                    showToast("error", "해당 회원을 찾을 수 없습니다.");
                } else {
                    showToast("error", "회원 상태 변경에 실패했습니다.");
                }
            } else {
